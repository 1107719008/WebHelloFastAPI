...
from router.schemas import InfoRequestSchema,UpdateInfoRequestSchema
...
def update_info_by_id(info_id: int,request:UpdateInfoRequestSchema, db: Session) -> DbHomework:
    updated_info = db.query(DbHomework).filter(DbHomework.id == info_id)#.first()
    if not updated_info:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f'Info Update with id = {info_id} not found')
    updated_info.update({
