...
from router.schemas import InfoRequestSchema, InfoResponseSchema,UpdateInfoRequestSchema,InfoResponseWithLikesCommentSchema
...
@router.get("/{author}", response_model=List[InfoResponseSchema])
def get_post_by_author(author: str, db: Session = Depends(get_db)):
    return db_info.get_post_by_author(username = author, db=db)
...
def update_info_by_id(id: int,request: UpdateInfoRequestSchema,db: Session = Depends(get_db)):
    return db_info.update_info_by_id(id=id,request=request, db=db)

@router.get('/id/{info_id}', response_model=InfoResponseWithLikesCommentSchema)
def get_info_by_id(info_id: int, db: Session = Depends(get_db)):
    return db_info.get_info_by_id(info_id=info_id, db=db)