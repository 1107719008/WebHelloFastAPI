...
@router.get("/id/{info_id}", response_model=List[InfoResponseSchema])
def get_info_by_id(id: int, db: Session = Depends(get_db)):
    return db_info.get_info_by_id(info_id=id, db=db)

@router.put("/{id}",response_model=List[InfoResponseSchema])
def update_info_by_id(id: int,db: Session = Depends(get_db)):
    return db_info.update_info_by_id(info_id=id, db=db)